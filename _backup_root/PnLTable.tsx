...
import api from '../api';
import { ChevronDown, Edit2, Save, X, Download, Search, Calculator, AlertTriangle } from 'lucide-react';
...
const translations = {
    pt: {
        ...
        subtitle: 'Clique em uma linha para ver detalhes das transações',
        ...
        validationAlerts: 'Alertas de Validação Matemática',
        grossProfitIncorrect: 'Lucro Bruto incorreto',
        expected: 'esperado',
        actual: 'atual'
    },
    en: {
        title: 'Profit & Loss Statement (P&L)',
        subtitle: 'Click on a row to see transaction details',
        loading: 'Loading P&L...',
        save: 'Save',
        cancel: 'Cancel',
        edit: 'Edit',
        ...
        validationAlerts: 'Mathematical Validation Alerts',
        grossProfitIncorrect: 'Gross Profit incorrect',
        expected: 'expected',
        actual: 'actual'
    }
};
    ...
    return (
        ...
        >
            ...
            {data.validation_alerts && data.validation_alerts.length > 0 && (
                <GlassCard className="bg-red-500/10 border-red-500/30 p-4">
                    <div className="flex items-center gap-2 text-red-400 mb-3">
                        <AlertTriangle size={20} />
                        <span className="font-semibold">{t.validationAlerts}</span>
                    </div>
                    <ul className="space-y-1 text-sm text-red-300">
                        {data.validation_alerts.map((alert, index) => (
                            <li key={index} className="flex items-center gap-2">
                                <span className="text-red-400">•</span>
                                <span>
                                    {alert.month}: {alert.field} {language === 'pt' ? 'incorreto' : 'incorrect'} ({t.expected}: R$ {alert.expected.toLocaleString('pt-BR')}, {t.actual}: R$ {alert.actual.toLocaleString('pt-BR')})
                                </span>
                            </li>
                        ))}
                    </ul>
